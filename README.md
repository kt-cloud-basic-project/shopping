...
#### 도입 배경
- 에러 발생 시 로그 확인이 늦어짐
- PR, 배포 이벤트가 팀 전체에 공유되지 않음

#### 개선 내용
- GitHub 이벤트 → Slack **실시간 알림 연동**
- PR 생성 / 머지 / 배포 알림 **자동화**
- 주문 실패, 결제 오류 등 **중요 도메인 에러 Bot 알림**
- 장애 상황에 대한 **즉각적인 팀 단위 대응 가능**

#### 효과
- 이슈 인지 시간 단축
- 운영 중 발생하는 문제에 대한 가시성 확보
...
#### 도입 배경
- 무분별한 API 호출로 인한 서버 부하 및 서비스 품질 저하 우려
...
**기존 문제**
- 멤버십과 할인 구조가 1:1로 강하게 결합
- 할인 로직이 복잡하여 다른 도메인 적용이 어려움

**개선 사항**
- 상품별 할인 구조 도입
- 할인 조합 가능 구조 설계
- 최대 할인 자동 계산 로직 추가
- 할인 계산 로직을 별도 서비스로 분리

**결과**
- 할인 정책 확장성 증가
- 다른 도메인에서도 재사용 가능한 구조 확보
...
**개선 내용**
- 배송지 미입력 시  
  → 유저의 기본 배송지 중 **가장 최근 주문에 사용된 배송지 자동 적용**
- 주문 요청 시 전달된 배송지 ID가  
  → **해당 유저 소유 배송지인지 검증 로직 추가**

**효과**
- 잘못된 배송 정보로 인한 주문 오류 방지
- 사용자 경험 개선
...
#### 기존 문제점
- 주문 / 결제 API가 분리된 트랜잭션 구조  
  → 결제 성공 후 재고 차감 실패 시 롤백 불가
- 재고 동시성 제어 부재  
  → 초과 판매 가능성 존재
...
#### 도입 배경
- 구매 전 탐색 단계에서의 **명확한 관심 데이터 확보**
- 할인 알림 등 비동기 알림 기능의 기준 도메인 필요
- 장바구니와 목적이 다른 **중·장기 관심 상품 관리 도메인 분리**

#### 효과
- 사용자 행동 데이터 정교화
- 할인 알림, 추천 기능 등 확장 기반 확보
- 도메인 책임 분리로 로직 명확성 향상
...
#### 도입 배경
- 할인 및 이벤트 발생 시, 사용자가 사이트에 접속 중이 아니더라도  
  정보를 전달받을 수 있도록 **사용자 편의성 향상**을 목표로 함

#### 해결 목표
- 찜한 상품 또는 장바구니에 담긴 상품에 할인 이벤트 발생 시  
  이메일 알림을 통해 **기간이 짧은 이벤트를 놓치지 않도록 유도**
- 찜 / 장바구니에 동시에 존재하는 상품에 대해  
  **중복 메일 발송 방지 로직 적용**
...
#### 팀 방향성
- 신규 AI 기능 확장보다는 **기존 서비스 내부 기능 강화**에 중점
- FAQ 외 NOTICE, POLICY 챗봇도 검토하였으나  
  팀 방향성과 일정 고려하여 **FAQ 챗봇 우선 개발** 결정

#### 개발 방식
- 강사님 제공 OpenAI 예제 코드 기반으로 개발
- 기존 구조를 그대로 사용하지 않고
  - 폴더 구조 세분화
  - 일부 로직 수정 및 책임 분리
...
#### 도입 배경
- 서비스 운영 단계에서 발생하는 에러 및 장애 원인을  
  단순 코드 로그만으로 추적하는 데 한계 존재
- 로그 수집 · 분석 · 시각화를 위해 **ELK 스택 도입**
- 이후 서버가 간헐적으로 다운되는 현상 발생
  - ELK 로그만으로는 원인 파악이 어려움
- CPU / 메모리 사용량, 컨테이너 리소스 상태 등  
  **시스템 전반의 상태를 함께 확인할 필요성 인지**
- 이에 따라 **Prometheus + Grafana** 추가 도입
